...
def main():
    ...
    p_value = [(0.4,0.2,0.3,0.1), (0.6,0.15,0.2,0.05), (0.6,0.3,0.07,0.03)]
    ...
    with open("results-nodes.csv", "w",) as f:
        writer = csv.writer(f)
        writer.writerow(["10000","20000","30000","40000","50000","trial", "p1", "p2", "q1", "q2"])


    with open("results-edges.csv", "w",) as file:
        writer2 = csv.writer(file)
        writer2.writerow(["10000","20000","30000","40000","50000","trial", "p1", "p2", "q1", "q2"])

    #runtime = []
    for p1,p2,q1,q2 in p_value:

        for t in range(1,10):
            print("p1 of: {}, trial: {}".format(p1, t))
            ...
            for i in range(3,50005):
                ...
                if G.number_of_nodes() == 0:
                    ...
                    G.add_node(float(2))
                    G.add_edge(1, 2)
                    G = addition_deletion(G, p1, p2, q1, q2)
                else:
                    G = addition_deletion(G, p1, p2, q1, q2)
            ...
            with open("results-nodes.csv","a") as f:
                writer = csv.writer(f)
                writer.writerow(y+[t]+[p1]+[p2]+[q1]+[q2])

            with open("results-edges.csv","a") as file:
                writer2 = csv.writer(file)
                writer2.writerow(z+[t]+[p1]+[p2]+[q1]+[q2])
